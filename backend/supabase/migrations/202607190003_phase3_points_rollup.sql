...
begin
  ...
  where public.battles.id = battle_uuid
    and public.battles.status = 'waiting'
  returning true, public.battles.started_at into started, started_at;
...
create or replace function public.add_college_points(college_uuid uuid, points_delta integer)
returns void
language plpgsql
set search_path = public
as $$
begin
  ...
  where public.colleges.id = college_uuid;
end;
$$;
