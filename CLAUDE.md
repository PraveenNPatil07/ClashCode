...
ClashCode is a real-time, competitive coding platform designed for university students. Represent your college, challenge rivals to 1v1 live programming battles, and solve algorithms faster than your opponent to climb the global leaderboard.
...
- Run dev: `npm run dev:backend` (Terminal 1) and `npm run dev:frontend` (Terminal 2)
- Test: `npm run test:unit` and `npm run test:e2e`
- Lint: N/A
- Build: `npm run build:frontend` (assuming Vite standard)
...
Frontend: React + Vite + TypeScript + TailwindCSS. Backend: Node.js + Express + TypeScript + Socket.io. Database: PostgreSQL via Supabase. Package manager: npm workspaces.
...
- **Branching** — state your project's model here, don't assume one. **Trunk-based** (commit straight to `main`, lean on tags + `/rewind`) is fine for a solo dev or a small repo; **feature-branch + PR** suits a team, review gates, or CI. Pick one, delete the other.
...
Project bootstrapped with [groundrules](https://github.com/lozit/groundrules) on 2026-07-20.
